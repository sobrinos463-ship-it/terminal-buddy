...
import { FreeTrainingSession } from "@/components/training/FreeTrainingSession";
...
export default function Training() {
  ...
  // Free training state
  const [freePhase, setFreePhase] = useState<'select' | 'train'>('select');
  const [selectedFreeExercises, setSelectedFreeExercises] = useState<SelectedExercise[]>([]);
  const [freeSessionId, setFreeSessionId] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchRoutine = async () => {
      ...
      // If free mode, we just set loading to false - session will be created when exercises are confirmed
      if (isFreeMode) {
        setLoading(false);
        return;
      }
  ...
  // Free training mode - Phase 1: Select exercises
  if (isFreeMode && freePhase === 'select') {
    const handleConfirmExercises = async (exercises: SelectedExercise[]) => {
      if (!user) return;
      
      try {
        // Create session when exercises are confirmed
        const { data: sessionData, error: sessionError } = await supabase
          .from('workout_sessions')
          .insert({
            user_id: user.id,
        ...
        setSelectedFreeExercises(exercises);
        setFreeSessionId(sessionData.id);
        setFreePhase('train');
      } catch (err) {
  ...
  // Free training mode - Phase 2: Training session
  if (isFreeMode && freePhase === 'train' && freeSessionId) {
    return (
      <MobileFrame>
        <MobileContent className="p-0">
          <FreeTrainingSession
            exercises={selectedFreeExercises}
            sessionId={freeSessionId}
            onBack={() => setFreePhase('select')}
          />
        </MobileContent>
      </MobileFrame>
    );
